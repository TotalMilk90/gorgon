...
IF ~Global("h_RigaldoQuest","GLOBAL",9)~ THEN BEGIN CARNIVAL_0
   SAY ~Aye, <CHARNAME>. We're on orders to await your signal. Whats the word then? Are we ready to make our move?~
       +~NumItemsParty("h_dart03",16)~+ ~Yes I've secured all their weapons. Divy them up and ready yourselves for action.~ GOTO CARNIVAL_4
...
IF ~~ THEN BEGIN CARNIVAL_1
   SAY ~Roger that. Alright you men! You heard the lass. Stand fast until she returns with the weapons.~ IF ~~ THEN EXIT
END

IF ~~ THEN BEGIN CARNIVAL_2
   SAY ~Roger that. Alright you men! You heard the lad. Stand fast until he returns with the weapons.~ IF ~~ THEN EXIT
END

IF ~~ THEN BEGIN CARNIVAL_3
   SAY ~Gods, what was Rigaldo thinking sending us a rookie for this mission. Go inside those tents there and steal the weapons off every gamesman inside, then bring them back to us. We will take it from there.~ IF ~~ THEN EXIT
END

IF ~~ THEN BEGIN CARNIVAL_4
   SAY ~Aye! Come on you men, take your weapons and let's move out!~
IF ~~ THEN DO ~
   SetGlobal("h_BaldwinMove","GLOBAL",1)
...
~Scores remain outside Beregost with Thamuz at its helm! They will be here by nightfall!~
   == h_baldwd
~Shit! So, they finally decided to make their move now, have they? Men! Ready your stations and prepare for battle!~
   =
