...
IF ~~ THEN BEGIN RIDDLE_0
   SAY ~Bored already, are you? You're starting to sound a little too much like Liedel, if you ask me. Never liked that woman to tell it true, got a funny smell about her.~IF ~~ THEN GOTO RIDDLE_0.5
END

IF ~~ THEN BEGIN RIDDLE_0.5
   SAY ~Anywho, my work ain't the kind you can just jump into, got it? However, I do have quite the thrilling game we could play, if you're looking to pass the time.~
       ++ ~A game, huh? What is it?~ GOTO RIDDLE_1
...
IF ~~ THEN BEGIN RIDDLE_3
   SAY ~Oh, this will be fun! Let us begin.~ IF ~~ THEN GOTO RIDDLE_4
END
...
IF ~~ THEN BEGIN RIDDLE_5
   SAY ~There are three priests who all say Destin is their brother. Yet, Destin says he has no brothers. Who is lying?~
       ++ ~Noone.~ DO ~AddExperienceParty(100)~ GOTO CORRECT_2
...
IF ~~ THEN BEGIN RIDDLE_6
   SAY ~I break, but never fall. And I fall, but never break. What are we?~
       ...
       ++ ~Two birds.~ GOTO WRONG_3
       ++ ~Day and night.~ DO ~AddExperienceParty(150)~ GOTO CORRECT_3
       ++ ~A sickness and cure.~ GOTO WRONG_3
       ++ ~Heartache and love.~ GOTO WRONG_3
...
IF ~~ THEN BEGIN END_0
   SAY ~I hope you had as much fun as I did. Time to go refill my Bag of Mysteries for my next victim!~
       ...
       ++ ~What am I supposed to do with all of this junk?~ DO ~SetGlobal("h_ZedaRiddle","GLOBAL",2)~ EXIT
...
IF ~~ THEN BEGIN NO_3
   SAY ~A tricky one, that was. The answer was "ton", as backwards it spells n-o-t.~
IF ~~ THEN DO ~
...
IF ~~ THEN BEGIN SECOND_11
   SAY ~I hope you enjoyed my new prizes, hehe!~
       ++ ~Yay! More junk that I don't need.~ DO ~SetGlobal("h_SecondRiddle","GLOBAL",2)~ EXIT
